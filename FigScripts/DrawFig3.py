...
def main():
    ...
    ax2=fig.add_axes([0.29,0.83,0.17,0.12])
    ...
    cbar.set_label('Core score', labelpad=0.15)  # Add label to the colorbar
    ...
    for i in range(0, 16):
        ...
        if i<8:
            ...
            if i==15:
                ...
                cax = fig.add_axes([cbar_left, ax_pos.y0-0.01, cbar_width, 0.005])
        ...
        for j in range(3):
            ...
            if i<4:
                ax3.annotate(NumberName[j]+f'={number:.2f}', (text_x, text_y - (j + 0.5) * step), fontsize=5, va='center')
            else:
                ax3.annotate(f'{number:.2f}', (text_x, text_y - (j + 0.5) * step), fontsize=5, va='center')
            if j!=2:
                ax3.annotate('_____', (text_x, text_y - (j + 0.5) * step-step2/2), fontsize=5, va='center')
    ...
    cax.set_xlabel('Spatial information rate (bit/s)', labelpad=1)
    ...
    ax6.plot([0,0,3,3],[8.8,9.0,9.0,8.8], color='k', linewidth=1)
    ax6.text(0.5,7.5, '****',ha='center')
    ax6.text(1.5,7.95, 'ns',ha='center')
    ax6.text(2.5,8.3, '**',ha='center')
    ax6.text(1.5,9.15, 'ns',ha='center')
    ...
    plt.figtext(axX3pos.x0-0.02,axX3pos.y0-0.005,'Spatial information rate (bit/s)',rotation=90,ha='center',va='center')
    axX3.set_title(r'$\mathit{HD}$')
    ...
    for m in ['Mouse12-120807','Mouse28-140313','Mouse12-120808' ]:
        for s in States:
            ...
            # 生成拟合的直线数据
            line = slope * np.array(core_score) + intercept
            # 画拟合的直线
            if p_value<0.01:
                if s=='Wake':   
                    axX10.plot(core_score, line, linestyle='-',c='tab:red', label='y={:.2f}x+{:.2f}'.format(slope,intercept),alpha=0.3)
                elif s=='REM':   
                    axX10.plot(core_score, line,  linestyle='--',c='tab:red', label='y={:.2f}x+{:.2f}'.format(slope,intercept),alpha=0.3)
                else:
                    axX10.plot(core_score, line,  linestyle=':',c='tab:red', label='y={:.2f}x+{:.2f}'.format(slope,intercept),alpha=0.3)
    ...
    for s in session_list:
        ...
        if p_value<0.01:
            axX11.plot(core_score_s, line, 'tab:red', label='y={:.2f}x+{:.2f}'.format(slope,intercept),alpha=0.5)
        else:
            axX11.plot(core_score_s, line, 'grey', label='y={:.2f}x+{:.2f}'.format(slope,intercept),alpha=0.5)
    ...
    axX3_pos = axX3.get_position()  
    plt.figtext(axX3_pos.x0-0.03,axX3_pos.y1+0.0,'h',fontsize=15)
