...
def main():
    ...
    # cell pair correlation分布的展示
    ax41=fig.add_axes([ax31_x+0.16,ax2_y,0.08,0.06])
    ax42=fig.add_axes([ax31_x+0.26,ax2_y,0.08,0.06])
    ax43=fig.add_axes([ax31_x+0.36,ax2_y,0.08,0.06])
    ...
    ax71=fig.add_axes([0.05,0.34,0.10,0.10/11*8.5])
    ax_pos71 = ax71.get_position()
    ax72=fig.add_axes([0.19,0.34,0.10,0.10/11*8.5])
    ax73=fig.add_axes([0.32,0.33,0.12,0.10])
    ax74=fig.add_axes([0.46,0.34,0.10,0.10/11*8.5])
    # 证明cp结构的存在
    ax81=fig.add_axes([0.60,0.34,0.10,0.10/11*8.5])
    ax82=fig.add_axes([0.73,0.34,0.10,0.10/11*8.5])
    ax83=fig.add_axes([0.86,0.34,0.10,0.10/11*8.5])
    ...
    for no, ax in enumerate(axes):
        ...
        if no<3:
            ax.set_title('H$_{%d}$         '%(no+1),fontsize=7.5,pad=0.1)
        else:
            ax.set_title('N$_{%d}$         '%(no-2),fontsize=7.5,pad=0.1)
    ...
    for no, ax in enumerate(axes):
        ...
        if no<3:
            ...
            ax.set_title('G$_%d$         '%(no+1),fontsize=7.5,pad=0.1)
            # if no==2:
    ...
    cbar.set_ticklabels(['-{:.1f}'.format(vrange), ' ', '0.0', '', '{:.1f}'.format(vrange)])
    ...
    ax54.set_ylabel('Cell pair #', labelpad=0.02)
    mid = m_all.shape[1] // 2
    ...
    cbar.ax.tick_params(labelsize=5)
    cbar.set_ticks([-vrange, -vrange/2, 0, vrange/2, vrange])
    cbar.set_ticklabels(['-{:.2f}'.format(vrange), ' ', '0.0', '', '{:.2f}'.format(vrange)])
    ...
    cbar.set_ticklabels(['-0.2', '0.0', '', '','0.6'])
    ...
    plt.figtext(ax_pos54.x0-0.04,ax_pos54.y1,'d',fontsize=15)
    ...
    plt.figtext(ax74_pos.x0-0.03,ax74_pos.y1,'h',fontsize=15)
    plt.figtext(ax_pos81.x0-0.03,ax_pos81.y1,'i',fontsize=15)
