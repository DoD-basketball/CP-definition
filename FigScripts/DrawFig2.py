...
def main():
    ...
    ax12.set_xticklabels(np.arange(12)+1)
    ax12.set_yticklabels(np.arange(12)+1)
    ...
    ax13.set_xticklabels(rearr+1)
    ax13.set_yticklabels(rearr+1)
    ax13.set_title('Shuffle with maximal $R_\\gamma$',pad=0.1)
    ...
    ax14.set_xticklabels(arr_shuffle+1)
    ax14.set_yticklabels(arr_shuffle+1)
    ...
    ax15.set_xticklabels(arr_shuffle+1)
    ax15.set_yticklabels(arr_shuffle+1)
    ...
    ax16.set_xticklabels(arr_shuffle+1)
    ax16.set_yticklabels(arr_shuffle+1)
    np.random.shuffle(arr_shuffle)
    im2s=im2[arr[arr_shuffle],:][:,arr[arr_shuffle]]
    ...
    ax17.set_xticklabels(arr_shuffle+1)
    ax17.set_yticklabels(arr_shuffle+1)
    ...
    ax18a.set_title('$C^*(\\gamma)$',fontsize=9)
    ...
    ax18b.set_title('$(C^{*})^{T}$',fontsize=9)
    ...
    ax13.text(5.5,21,'Iterating over $\\alpha$ and $\\beta$,\nrepeating Steps III-V',fontsize=6, ha='center', va='center')
    ax13.text(5.5,25,'$\\downarrow$',fontsize=15, ha='center', va='center')
    ax13.text(5.5,29,'$S_{C,i}=Z\\sum_\\gamma C_i^*(\\gamma)R_\\gamma$',fontsize=9, ha='center', va='center')
    ...
    plt.figtext(ax13_pos.x0-0.015,ax13_pos.y1+0.005, r'$\mathrm{V}$',fontsize=10)
    plt.figtext(ax13_pos.x0-0.015,ax12_pos.y1+0.005, r'$\mathrm{VI}$',fontsize=10)
    ax14_pos = ax14.get_position()
    plt.figtext(ax14_pos.x0-0.015,ax14_pos.y1+0.005, r'$\mathrm{IV}$',fontsize=10)
    ...
    for no,p in enumerate(path):
        ...
        if no==0:
            ...
            cax_x = ax21_position.x1+0.003   # 在 ax21 的右侧偏移一定距离
            ...
            cbar.set_label('Core score',labelpad=1)  # Add label to the colorbar
        ...
        ax23.set_ylabel('% of cells in class')
